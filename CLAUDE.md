...
⚠️ **migration_security_hardening.sql (raiz): PRECISA ser executada — correções da auditoria de segurança.** Fecha falhas CONFIRMADAS por teste contra o banco real: (1) usuário alterava o próprio `role` e virava Diretor; (2) bucket de fotos sem isolamento por hospital — o Inspetor de um hospital gravava e apagava foto na pasta de outro; (3) Supervisor validava relatório (só a UI barrava); (4-9) `hospital_id` forjável em reports/inspections/audit_log, vínculo e pedido de acesso entre hospitais. Idempotente, não muda regra de negócio. Rodar DEPOIS de supabase_setup.sql
⚠️ migration_demo_reset.sql: **REEXECUTAR** — o Supabase passou a bloquear `DELETE FROM storage.objects` ("Use the Storage API instead") e, como era o PRIMEIRO comando, o erro abortava a transação e a limpeza NÃO apagava nada. Agora o bloco das fotos é tolerante à recusa (devolve `fotos_storage = -1`) e o Painel Demo remove as fotos pela Storage API. Também passou a limpar `invitations`, que sobrevivia à limpeza
...
✅ LOGOUT NÃO APAGA O CACHE (set/2026): signOut() chamava clearProfile()+clearWorkData(), então "Continuar offline" era impossível depois de sair. Agora markSignedOut() bloqueia só a ENTRADA AUTOMÁTICA e o cache é preservado; a limpeza acontece em _loadProfile() quando OUTRO uid entra com senha. NÃO reintroduzir a limpeza no signOut
✅ TAXA DE CONFORMIDADE, FONTE ÚNICA (set/2026): core/utils/compliance_utils.dart. Agregação por ITEM — C/(C+NC), NA fora do denominador. A média das compliance_rate (que estava nos cards) dá o mesmo peso a uma inspeção de 2 itens e a uma de 50: 70,0% contra 88,5% nos mesmos dados. O card duplicado saiu das 3 telas; a taxa vive só no donut
✅ Contador da série (set/2026): TaskGroup.total vem de series_total, NÃO de tasks.length — o painel do Inspetor consulta só pending/in_progress e mostrava "0 de 3" depois de responder a primeira de 4
✅ Contraste auditado por render (set/2026): test/chip_render_audit_test.dart monta cada componente e mede WCAG. Único achado: sentLight (#22C55E) rende 2,28:1 e deixava "Enviado" apagado — o StatusBadge usa validatedDark no TEXTO, mantendo a cor do status no fundo. Paleta intocada
✅ Saudação (set/2026): nome igual ao cargo não repete ("Diretor Demo", não "Diretor Diretor") — as contas demo se chamam assim
✅ ORIGEM DO DADO CENTRALIZADA (set/2026): core/services/data_source.dart. OFFLINE lê do local e NÃO dispara chamada de rede (não é falhar e cair no cache, é não tentar); ONLINE lê da rede e atualiza o cache. Toda tela do Inspetor tem guarda ANTES do try. Sem equivalente local ficam OCULTOS offline: criar inspeção, finalizar, histórico e notificações. NÃO reintroduzir consulta direta nas telas
✅ AuthProvider._offlineMode virou SETTER que espelha o DataSource — eram 9 atribuições espalhadas e esquecer uma fazia uma tela voltar a consultar a rede sem conexão
✅ CHIP: widget/app_filter_chip.dart, estilo NO WIDGET (WidgetStateProperty), não no tema. O chipTheme do M3 não alcançava FilterChip/ChoiceChip de forma confiável — três tentativas falharam. Todo chip do app usa AppFilterChip. NÃO voltar a estilizar chip pelo tema
✅ PDF com fontes TTF embutidas (set/2026): pw.ThemeData.withFont com Manrope (corpo) e Sora (fallback). A Helvetica padrão do pacote pdf não tem glifo para — (U+2014) nem · (U+00B7), que saíam como quadrado com X. Verificado nos bytes: /BaseFont /Manrope-Regular com FontFile2 e nenhuma /Helvetica
✅ Série agrupada em TODAS as telas que listam ocorrências (set/2026): painel do Inspetor, série, quadro de gestão e aba Tarefas do setor. Os calendários filtram por DIA (série não repete ali) e trazem só o selo "3 de 14"
✅ AUDITORIA DE SEGURANÇA (set/2026): docs/AUDITORIA_SEGURANCA.md — 78 políticas RLS lidas E testadas contra o banco real com a anon key pública. O isolamento de LEITURA entre hospitais resistiu a tudo (inclusive contagem e exportação); anônimo não lê nada. As falhas eram de ESCRITA e de PRIVILÉGIO: `profiles_update_own` não restringia colunas, então Inspetor e Supervisor se promoviam a Diretor com um PATCH; e o bucket de fotos só exigia "estar autenticado". Correções em migration_security_hardening.sql. Os testes que promoveram/validaram foram REVERTIDOS na hora e o estado conferido
✅ D2 — nenhuma mensagem técnica na tela (set/2026): 15 pontos exibiam `${e.message}` do Postgres ao usuário. Agora a tela diz o que fazer e o detalhe vai para o debugPrint. Também saíram da interface "Verifique as políticas RLS", "execute migration_task_series.sql no SQL Editor" e a menção ao Supabase na confirmação de e-mail
✅ TAXA GRAVADA = TAXA EXIBIDA (set/2026): ao ENVIAR a inspeção, resposta_checklist gravava `compliant / _items.length * 100`, com os NA no denominador — 5 C, 0 NC e 5 NA viravam 50% no banco enquanto o donut mostrava 100%. Agora usa ComplianceUtils. As cópias da fórmula em charts.dart e report_export_service.dart (que estavam CERTAS, mas duplicadas) passaram a chamar a fonte única. test/compliance_utils_test.dart cobre o caso
✅ SALVAR OFFLINE É SALVAR (set/2026): a tela de resposta tinha DUAS filas — uma em memória e a de disco — que drenavam a mesma inspeção sem se conhecer; era daí o risco de perder resposta ao alternar entre checklists. A de memória saiu. `SaveState.savedLocal` distingue "gravado no aparelho, falta rede" de erro: sair offline não alarma mais, só confirma o que ficou guardado. Reabrir um checklist respondido offline agora repõe as respostas (`OfflineStore.queuedResponses` mescla a fila sobre o pacote baixado, nos dois caminhos de abertura). A faixa só diz "Enviando" com envio REALMENTE em curso (`OfflineSyncService.syncing`); com fila parada diz "aguardando envio", e `syncIfNeeded()` drena o que sobrou de sessão anterior
✅ CHIP SEM FILTERCHIP (set/2026, 6ª tentativa — a que pegou): o FilterChip perde a resolução do WidgetStateTextStyle ao fazer merge com o estilo do tema, e o texto caía para branco. AppFilterChip foi reescrito do ZERO — InkWell + Ink + Text com cor explícita, nenhum widget Chip. O chipTheme órfão foi podado (sobrou só o mínimo dos Chip informativos da gestão de usuários). NÃO voltar para FilterChip. O teste antigo lia `chip.color.resolve(...)`, ou seja, a configuração — passava com o app quebrado; agora mede o pixel (cor do Text e do Ink renderizados) e há um caso com chipTheme hostil provando que nada vem do tema
✅ SÉRIE AGRUPADA EM RELATÓRIOS E HISTÓRICO (set/2026): faltavam essas duas. Relatórios & Análises agrupa por série ANTES de agrupar por data (as ocorrências caem em dias diferentes; o grupo se posiciona pela mais recente) — card expansível com taxa agregada por ITEM, total de NCs e selo "3 de 14"; filtros e busca seguem agindo antes, sobre relatórios individuais. Histórico do Inspetor idem. Os calendários continuam por DIA, de propósito
✅ historico_screen filtrava por inspector_id mas NÃO por hospital_id (set/2026) — a RLS já cobria, mas a regra de ouro existe para a proteção não depender de uma camada só. Corrigido
🔧 Testes: test/task_series_utils_test.dart (10 casos, inclui o mês curto do 31/01), test/offline_store_test.dart (I/O real do cache offline), test/profile_name_test.dart, test/offline_fila_por_inspecao_test.dart (A1: fila por inspeção, dois checklists offline), test/login_layout_test.dart test/task_status_test.dart, test/task_series_group_test.dart, test/compliance_utils_test.dart, test/chip_render_audit_test.dart, test/roteiro_offline_test.dart e test/roteiro_offline_fluxo_test.dart (roteiros offline passo a passo), test/app_filter_chip_test.dart, test/pdf_glifos_test.dart e test/l10n_test.dart — 120 no total (chip_contrast_test.dart foi REMOVIDO: testava o chipTheme que deixou de existir). Cobertura ainda parcial: o grosso das telas continua sem teste